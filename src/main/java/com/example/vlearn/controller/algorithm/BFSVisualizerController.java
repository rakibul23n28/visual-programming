 ...
 */
public class BFSVisualizerController {
    ...
    private void handleAddEdge() {
        ...
        edge.weightText.setFill(Color.YELLOW);
    ...
    private void handleRunBFS() {
        ...
        if (startLabel.isEmpty()) {
            showMessage("Enter start node label.");
            return;
        }
        if (!nodes.containsKey(startLabel)) {
            showMessage("Start node not found.");
            return;
        }
            ...
            for (GraphEdge e : adjacency.get(u.id)) {
                ...
                if (!visited.contains(v)) {
                    ...
                    Platform.runLater(() -> {
                        ...
                        highlightNodeCircle(v.circle, Color.web("#FFFF00"));
